...
import com.voiceassistant.ruta.data.ApiClient
...
class ChatGptViewModel : ViewModel(){
    ...
    fun  takeLastMessage(): String{
        val currentList = _messageList.value ?: mutableListOf()
        val number = currentList.count()
        val result = currentList[number-1].message.toString()
        return result
    }
    ...
    fun callApi(question : String, speech: Speech){
        ...
        viewModelScope.launch {
            try {
                ...
                val text = takeLastMessage()

                speech.speak(text.substring(2))
